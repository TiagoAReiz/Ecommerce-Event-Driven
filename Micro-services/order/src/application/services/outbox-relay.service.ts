...
export class OutboxRelayService {
  ...
  private async relayOne(event: OutboxEvent): Promise<void> {
    ...
    try {
      // key = aggregateId = orderId → garante ordem entre OrderCreated/OrderReadyForPayment/
      // OrderCancelled do MESMO pedido no tópico order-events.
      await this.eventPublisher.publish(ORDER_EVENTS_TOPIC, event.aggregateId, JSON.stringify(envelope));
