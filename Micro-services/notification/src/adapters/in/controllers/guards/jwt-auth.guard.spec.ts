...
describe('JwtAuthGuard', () => {
  it('allows the request and attaches the decoded payload when the token is valid', async () => {
    const jwtService = {
      verifyAsync: jest.fn().mockResolvedValue({ sub: 'user-1', email: 'a@b.com', role: 'CUSTOMER' }),
    } as any;
  ...
  it('rejects when the token fails verification', async () => {
    const jwtService = { verifyAsync: jest.fn().mockRejectedValue(new Error('bad token')) } as any;
    const guard = new JwtAuthGuard(jwtService);
